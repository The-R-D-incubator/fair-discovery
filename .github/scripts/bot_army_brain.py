...
def forge_intel_report():
    # THE INTELLIGENCE LIBRARY (Keep this as-is)
    briefings = [
        {
    ...
    # THE SECURE, HIGH-CONTRAST TEMPLATE
    html_template = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        /* CRITICAL: Changed default text color to higher contrast slate-100 */
        body {{ font-family: 'Geist', sans-serif; background-color: #020617; color: #cbd5e1; line-height: 1.6; }}
        .glow-border {{ border: 1px solid rgba(45, 212, 191, 0.1); }}
        /* ADJUSTED: Gradient changed to end with white, maximizing contrast */
        .gradient-text {{ background: linear-gradient(135deg, #2dd4bf 0%, #fff 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }}
    </style>
</head>
<body class="min-h-screen py-12 px-6 md:py-24">
    <div class="max-w-3xl mx-auto">
        ...
        <header class="mb-16">
            <span class="px-3 py-1 bg-teal-500/10 text-teal-300 text-[9px] font-black rounded-full border border-teal-500/20 uppercase tracking-[0.2em] mb-6 inline-block">{tag}</span>
            <h1 class="text-5xl md:text-7xl font-black italic uppercase tracking-tighter leading-[0.9] mt-4 mb-8 gradient-text">
                {title}
            </h1>
        </header>

        <article class="text-lg text-slate-100 font-light">
            <h2 class="text-white font-bold text-2xl mb-6 italic uppercase tracking-tight">{headline}</h2>
            <p>{content}</p>

            <div class="glow-border bg-slate-900/60 p-8 rounded-[2rem] my-12 italic border-l-4 border-l-teal-500">
                <p class="text-white text-md mb-0 italic">"Forensic Insight: {insight}"</p>
            </div>
        ...
        <div class="mt-20 p-12 bg-white rounded-[3.5rem] text-center">
            ...
            <div class="flex flex-col sm:flex-row gap-3">
                ...
                <button onclick="saveLead()" class="bg-teal-500 text-white px-12 py-5 rounded-2xl font-extrabold uppercase text-[11px] tracking-widest">Initialize</button>
            </div>
...
"""
    ...
    # SAFER INJECTION logic remains the same
